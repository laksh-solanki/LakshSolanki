...
<template>
  <v-container class="py-8 px-4 h-100 max-w-xl mx-auto ppt-container">
    ...
    <div v-else class="editor-layout d-flex flex-column">
      ...
      <div class="d-flex align-center justify-center mt-6 bg-white rounded-pill elevation-2 py-2 px-6 mx-auto w-100 max-w-sm">
        <v-btn icon="mdi-chevron-left" variant="flat" color="grey-lighten-3" size="small" @click="activeSlideIndex--" :disabled="activeSlideIndex === 0"></v-btn>
        <span class="font-weight-bold px-6 text-subtitle-1">Slide {{ activeSlide?.slideNumber || 1 }} / {{ slidesData.length }}</span>
        <v-btn icon="mdi-chevron-right" variant="flat" color="grey-lighten-3" size="small" @click="activeSlideIndex++" :disabled="activeSlideIndex === slidesData.length - 1"></v-btn>
      </div>
    </div>
  </v-container>
</template>
